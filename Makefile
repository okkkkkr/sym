...
.PHONY: dev-admin
dev-admin: ## Starts the admin frontend dev server
	cd web && pnpm dev --host 0.0.0.0

.PHONY: dev-public
dev-public: ## Starts the public frontend dev server
	cd frontend && pnpm dev --host 0.0.0.0 --port 3200

.PHONY: build-admin
build-admin: ## Build the admin frontend
	cd web && pnpm build

.PHONY: build-public
build-public: ## Build the public frontend
	cd frontend && pnpm build

.PHONY: build-frontends
build-frontends: build-admin build-public ## Build both admin and public frontends
