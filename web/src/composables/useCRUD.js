...
export default function ({ name, initForm = {}, doCreate, doDelete, doUpdate, refresh }) {
  ...
  /** 删除 */
  async function handleDelete(params = {}, options = {}) {
    ...
    try {
      ...
      const successMessage =
        typeof normalizedOptions.successMessage === 'function'
          ? normalizedOptions.successMessage(data)
          : normalizedOptions.successMessage
      $message.success(successMessage || '删除成功')
      modalLoading.value = false
      refresh(data)
      normalizedOptions.onSuccess && normalizedOptions.onSuccess(data)
    } catch (error) {
      modalLoading.value = false
    }
  }
